...
class ChatbotUI(QWidget):
    ...
    def setup_ui(self):
        ...
        self.setStyleSheet('background-color: #fcfcfc; border: 1px solid #e8e8e8;')
        ...
        # Language selector dropdown with auto-detect
        lang_layout = QHBoxLayout()
        lang_layout.setContentsMargins(0, 0, 0, 0)
        self.language_selector = QComboBox(self)
        self.language_selector.addItems(['Auto-detect', 'Python', 'Java', 'C++', 'C'])
        self.language_selector.setStyleSheet('font-size: 9px; border: 1px solid #efefef; border-radius: 2px; padding: 2px; background: #fefefe; color: #666;')
        self.language_selector.setToolTip('Select language or Auto-detect from problem (for code problems only)')
        lang_layout.addWidget(self.language_selector)
        self.layout.addLayout(lang_layout)
        
        # Minimal stats indicator (hidden mostly)
        self.stats_label = QLabel('●')
        self.stats_label.setStyleSheet('color: #999; font-size: 8px; background: transparent; padding: 0px;')
        self.stats_label.setAlignment(Qt.AlignCenter)
        self.stats_label.setMaximumHeight(12)
        ...
        self.text_input.setStyleSheet('background: #fefefe; border: 1px solid #efefef; color: #555; font-size: 10px;')
        self.layout.addWidget(self.text_input)
        
        # Button container for send and toggle
        button_container = QHBoxLayout()
        
        # Add subtle send button (light, blends in)
        self.send_btn = QPushButton('↑', self)
        self.send_btn.clicked.connect(self.on_send_edited_question)
        self.send_btn.setStyleSheet('background-color: #f8f8f8; border: 1px solid #efefef; color: #aaa; font-size: 8px; padding: 2px 4px; border-radius: 2px; font-weight: normal;')
        self.send_btn.setToolTip('Send edited question')
        self.send_btn.setMaximumHeight(18)
        self.send_btn.setMaximumWidth(22)
        button_container.addWidget(self.send_btn)
        
        # Add toggle button (fallback for Alt+X in locked environments)
        self.toggle_btn = QPushButton('◄◄ Hide', self)
        self.toggle_btn.clicked.connect(self.toggle_system)
        self.toggle_btn.setStyleSheet('background-color: #f8f8f8; border: 1px solid #efefef; color: #999; font-size: 9px; padding: 4px 8px; border-radius: 2px; font-weight: normal;')
        self.toggle_btn.setToolTip('Toggle window visibility (Alt+X fallback)')
        self.toggle_btn.setMaximumHeight(18)
        button_container.addWidget(self.toggle_btn, 1)
        ...
        self.output.setStyleSheet('background: #fefefe; border: 1px solid #efefef; color: #666; font-size: 10px;')
        self.layout.addWidget(self.output)
        # Copy ID button hidden but functionality kept (for hotkey if added)
        ...
        self.room_entry.setPlaceholderText('Room code')
        self.room_entry.setMaximumHeight(18)
        self.room_entry.setStyleSheet('background: #fefefe; border: 1px solid #efefef; color: #666; font-size: 9px; padding: 2px;')
        
        # Join button (small)
        self.join_chat_btn = QPushButton('Join', self)
        ...
        self.join_chat_btn.setStyleSheet('background-color: #f8f8f8; border: 1px solid #efefef; color: #999; font-size: 8px; padding: 2px;')
        
        # Chat display (mini)
        self.chat_display = QTextEdit(self)
        self.chat_display.setReadOnly(True)
        self.chat_display.setMaximumHeight(60)
        self.chat_display.setStyleSheet('background: #fefefe; border: 1px solid #efefef; color: #666; font-size: 8px;')
        
        # Message input (small)
        self.msg_entry = QLineEdit(self)
        self.msg_entry.setPlaceholderText('msg')
        self.msg_entry.setMaximumHeight(18)
        self.msg_entry.setStyleSheet('background: #fefefe; border: 1px solid #efefef; color: #666; font-size: 9px; padding: 2px;')
        ...
        self.send_chat_btn.setStyleSheet('background-color: #f8f8f8; border: 1px solid #efefef; color: #aaa; font-size: 8px; padding: 2px 4px;')
    ...
    def start_native_hotkeys(self):
        ...
        hotkeys = {
            0x5841: ('alt_x', MOD_ALT, 0x58, 'toggle_system', 'Alt+X'),
            ...
            try:
                ...
                def on_press(key):
                    try:
                        ...
                        if hasattr(key, 'vk'):
                            # Alt+X is handled by native RegisterHotKey above to avoid double toggles.
                            if key.vk == 88 and alt_pressed:
                                if 'alt_x' in self.native_registered_hotkeys:
                                    return
                                self.trigger_hotkey_action('alt_x', 'toggle_system', 'Alt+X')
                ...
                print('  🔑 HOTKEYS:')
                print('     Alt+X - Toggle window visibility')
    ...
    def display_response(self, response, is_coding):
        ...
        print(f'[RESPONSE STORED] {len(response)} chars stored. Press Alt+X to view.')
    ...
    def setup_tray_icon(self):
        ...
        try:
            ...
            tray_menu.addAction('Toggle (Alt+X)', self.toggle_system)
            tray_menu.addSeparator()
    ...
    def toggle_system(self):
        """Toggle window visibility with Alt+X - shows stored response when unhiding"""
        try:
            if self.isVisible():
                self.hide()
